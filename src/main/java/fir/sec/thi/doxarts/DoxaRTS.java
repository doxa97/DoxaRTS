        ...
        getServer().getScheduler().scheduleSyncRepeatingTask(this, () -> {
            ...
            for (Player player : getServer().getOnlinePlayers()) {
                ...
                if (EquipWeapon.getType() != Material.STICK){
                    ...
                    weapon.put("drain", 0);
                    weapon.put("crit", 0);
                    weapon.put("attackspeed", 1);
                }
                else if (EquipWeapon.getType() == Material.STICK){
                    ItemMeta meta = EquipWeapon.getItemMeta();
                    for (int i = 0; i <= meta.getLore().size(); i++){
                        if (meta.getLore().get(i).contains("공격력 :")){
                            weapon.put("damage", Integer.valueOf(meta.getLore().get(i).replace(" ","")
                                    .replace("공격력","").replace(":","")));
                        }
                        if (meta.getLore().get(i).contains("공격 속도 :")){
                            weapon.put("attackspeed", Integer.valueOf(meta.getLore().get(i).replace(" ","")
                                    .replace("공격","").replace(":","").replace("속도","")));
                        }
                        if (meta.getLore().get(i).contains("흡혈 :")){
                            weapon.put("drain", Integer.valueOf(meta.getLore().get(i).replace(" ","")
                                    .replace("흡혈","").replace(":","")));
                        }
                        if (meta.getLore().get(i).contains("치명타 확률 :")){
                            weapon.put("crit", Integer.valueOf(meta.getLore().get(i).replace(" ","")
                ...
                Objects.requireNonNull(player.getAttribute(Attribute.GENERIC_ATTACK_SPEED)).setBaseValue((double) weapon.get("attackspeed"));
